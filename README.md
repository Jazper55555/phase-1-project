...
Within the webpage, ALL of the materials that can be found in *BOTW* are displayed as pictures. Simply hovering over an image with the mouse will reveal that images name as well as a description of the selected item. When the mouse leaves an image, the text dissapears. When a user *clicks* an image, locations for where that material may be found are revealed. The user can click again to make that information dissapear in order to reveal location information for a different image. The idea is relatively simple and gives users the ability to find relevent information on requested items as needed.
...
3. The 4 event listeners are as follows:
    1. 'DOMContentLoaded'
    2. 'mouseover'
    3. 'mouseout'
    4. 'click'
